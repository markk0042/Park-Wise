...
export default function QuickRegistrationLogWithSection({ section }) {
  ...
  const { data: vehicles = [], isLoading: vehiclesLoading } = useQuery({
    ...
    enabled: user?.status === 'approved', // Allow all approved users
  ...
  const handleSearch = () => {
    ...
    console.log('Sample vehicles:', vehicles.slice(0, 3).map(v => v.registration_plate));

    // First, try exact registration match
    ...
    if (!vehicle) {
      vehicle = vehicles.find(v => {
        ...
        if (regUpper.includes(searchTerm)) {
        ...
        const partMatch = regParts.some(part => part === searchTerm);
        if (partMatch) {
