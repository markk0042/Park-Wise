...
export default function CategoryReport({ logs, category, onBack, currentUser }) {
  ...
  const downloadCSV = () => {
    ...
    const summary = [
      ...
      [`Total Vehicles Logged: ${sortedLogs.length}`],
      [`Generated: ${format(new Date(), "dd/MM/yyyy HH:mm")}`],
      [""],
      ["VEHICLE LOGS (Sorted by Permit Number: Lowest to Highest)"],
      [""]
    ];
    ...
    link.href = url;
    link.download = `ParkingLog_${category}_Report_${format(new Date(), "yyyy-MM-dd")}.csv`;
